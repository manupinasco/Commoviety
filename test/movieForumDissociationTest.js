...
describe('Movie and Forum Dissotiation', () => {
    ...
    before('Create Movie and Forum', () => {
        ...
        return Promise.all([createForum, createMovie].map(fn => fn())).then(associateMovieForum)
    })


    it('Returns 201 if Movie and Forum are dissociated', (done) => {
    ...
    it("Returns 422 if MovieForum doesn't exist", (done) => {
        axios({
            method: 'delete',
            url: 'http://localhost:4444/moviesForums',
            data: { idMovie: idMovie, idForum: idForum }
            ...
            .catch(err => {
                assert.equal(err.response.data.message, 'MOVIEFORUM_DOESNT_EXIST')
                done()
            })
    })
